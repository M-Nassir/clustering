        ...
        while cco is True and itr < self.max_n_iterations_:
            ...
            for i in self.cluster_numbers_:
                ...
                if (X[y == i].shape[0] >= 2) and \
                    ...
                    # take sample of anomalies so cluster are grown slowly
                    M = X[y == -1]
                    ind_y = np.where(y == -1)

                    n = int(np.ceil(self.anomaly_sample_p_ * M.shape[0]))

                    indices = np.random.choice(M.shape[0], n, replace=False)

                    anomaly_points = M[indices, :]
                    ind0 = ind_y[0][indices]

                    # print(M.shape[0])
                    # print(anomaly_points.shape[0])
                    # print(ind0.shape)
